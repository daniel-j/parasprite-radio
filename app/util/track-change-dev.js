...
fs.writeFile(__dirname+'/now2/json', JSON.stringify(json), 'utf8')

console.log("Generating art...")
imageFromFile(liq.filename, function (err, type, data) {
	if (err) {
		console.log("Failed to generate art! "+err)
		fs.unlink(__dirname+'/now2/image-full', function () {})
		fs.unlink(__dirname+'/now2/image-small.png', function () {})
		fs.unlink(__dirname+'/now2/image-tiny.png', function () {})
		fs.unlink(__dirname+'/now2/type.txt', function () {})
	}
	else {
		

		fs.writeFile(__dirname+'/now2/image-full', data, function (err) {
			if (err) throw err
			console.log("Saved full art")
		})
		fs.writeFile(__dirname+'/now2/type.txt', type)
		...
		lwip.open(data, t, function (err, image) {
			if (err) console.log(err)
				...
				image.batch()
					.cover(80, 80)
					.writeFile(__dirname+'/now2/image-tiny.png', function (err) {
						if (err) throw err
		...
		lwip.open(data, t, function (err, image) {
			if (err) console.log(err)
				...
				image.batch()
					.cover(350, 350)
					.writeFile(__dirname+'/now2/image-small.png', function (err) {
						if (err) throw err
